...
import blackJack_main as bj_main
...
def game_menu(gambler, parent, bal_var):
    ...
    tk.Button(game_win, text="Play Blackjack", width=20,
              command=lambda: bj_main.blackjackGUI(gambler, game_win, bal_var)).pack(pady=8)
...
def manager_menu(username, parent):
    ...
    tk.Button(win, text="View Game Traffic", width=25, command=game_traffic).pack(pady=6)
    tk.Button(win, text="View Game Revenue", width=25, command=gameRevenue).pack(pady=6)
...
def gameRevenue():
    db = sqlite3.connect("casino8.db")
    cr = db.cursor()


    game_data = cr.execute("""
                           SELECT GAME,
                                  SUM(AMOUNT_BET) as total_bets,
                                  SUM(AMOUNT_WON) as total_payouts,
                                  COUNT(*)        as game_count
                           FROM GAME_HISTORY
                           GROUP BY GAME
                           ORDER BY GAME
    ...
    for row in game_data:
        ...
        total_bets = row[1]
        total_payouts = row[2]
        revenue = total_bets - total_payouts  # Calculate profit

        games.append(game_name.title())
        revenues.append(revenue)

    fig, ax = plt.subplots(figsize=(10, 6))


    bar_colors = ['red', 'blue', 'green', 'yellow'][:len(games)]

    bars = ax.bar(games, revenues, color=bar_colors, alpha=0.7)


    for bar, revenue in zip(bars, revenues):
        height = bar.get_height()
        ax.text(bar.get_x() + bar.get_width() / 2., height + height * 0.01,
                f'${revenue:.2f}', ha='center', va='bottom', fontweight='bold')

    ax.set_ylabel('Profit ($)')
    ax.set_xlabel('Games')
    ax.set_title('Games and Their Profit')
...
def player_winnings_losses():
    ...
    stats = cr.execute("""
                       SELECT SUM(MONEY_WON)  as total_won,
                              SUM(MONEY_LOST) as total_lost
                       FROM GAMBLER
                       """).fetchone()
    ...
    money_won = stats[0] or 0
    money_lost = stats[1] or 0


    sizes = [money_won, money_lost]
    labels = ['Money Won', 'Money Lost']
    colors = ['green', 'red']
    ...
    ax.set_title('Player Winnings vs Losses\n(All Players Combined)', fontsize=14, fontweight='bold')


    total = money_won + money_lost
    plt.figtext(0.5, 0.02, f'Total Won: ${money_won:.2f} | Total Lost: ${money_lost:.2f}',
...
def game_traffic():
    ...
    if not game_data:
        print("No game traffic data available yet!")
        db.close()
        return
    ...
    for row in game_data:
        ...
        games.append(game_name.title())
        counts.append(game_count)

    # Create the chart
    fig, ax = plt.subplots(figsize=(10, 6))

    bar_colors = ['red', 'blue', 'green', 'yellow'][:len(games)]
    bars = ax.bar(games, counts, color=bar_colors, alpha=0.7)
    ...
    ax.set_xlabel('Games')
    ax.set_title('Game Traffic - Number of Games Played by Type')
    ax.grid(axis='y', alpha=0.3)

    # Add legend showing total games
    total_games = sum(counts)
    ax.text(0.02, 0.98, f'Total Games: {total_games}', transform=ax.transAxes,
...
def individual_player_stats(player_id):
    ...
    player_data = cr.execute("""
                             SELECT FIRST_NAME, LAST_NAME, MONEY_WON, MONEY_LOST, WINS, LOSSES
                             FROM GAMBLER
                             WHERE ID = ?
                             """, (player_id,)).fetchone()
    ...
    first_name, last_name, money_won, money_lost, wins, losses = player_data

    # Create pie chart for this player
    sizes = [money_won, money_lost]
    labels = ['Money Won', 'Money Lost']
    colors = ['green', 'red']
    ...
    ax.set_title(f'{first_name} {last_name} - Winnings vs Losses\n'
                 f'Games: {wins} wins, {losses} losses',
