...
import random
#Windows logo key and period to pull up emoji
# symbols & payouts
symbols = ["🍒", "❤️", "🍇", "🍧", "🍓", "😍"]
payouts = {
    "🍒": 2,
...
class SlotMachine:
    def __init__(self, root, starting_balance=100):
        ...
        # title banner
        title_label = tk.Label(root, text="🎰 SLOT MACHINE 🎰", font=("Arial", 28), fg="pink", bg="#222")
        title_label.pack(pady=10)

        # frame for the reels
        reel_frame = tk.Frame(root, bg="white", bd=5, relief="ridge")
        reel_frame.pack(pady=10)

        self.reel_labels = []
        for _ in range(3):
            label = tk.Label(reel_frame, text="❔", font=("Arial", 48), width=2, bg="white", fg="black")
            label.pack(side="left", padx=10)
            self.reel_labels.append(label)

        # balance shown and betting frame
        ...
        # buttons such as the spin and cash out
        button_frame = tk.Frame(root, bg="#222")
        button_frame.pack(pady=10)

        self.spin_button = tk.Button(button_frame, text="🔃 Spin", font=("Arial", 12, "bold"), command=self.spin, bg="grey", fg="white", width=10)
        self.spin_button.grid(row=0, column=0, padx=10)

        self.cashout_button = tk.Button(button_frame, text="💰 Cash Out", font=("Arial", 12, "bold"), command=self.cash_out, bg="pink", fg="white", width=10)
    ...
    def spin(self):
        ...
        if self.balance == 0:
            messagebox.showinfo("Game Over", "You don't have any money left to bet with! Game Over.")
            self.root.quit()
...
class StartWindow:
    def __init__(self, root):
        ...
        tk.Button(root, text="Start Game", command=self.start_game, font=("Arial", 12, "bold"), bg="pink", fg="white").pack(pady=10)
