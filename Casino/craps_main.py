...
def play_craps(gambler, parent, bal):
    ...
    def roll_dice():
        ...
        if won is not None:
            ...
            if loss_streak[0] >= 3:
                cheat_checkbox.config(state='normal')
            else:
                cheat_checkbox.config(state='disabled')
                cheat_var.set(0)
    ...
    # Cheat checkbox (disabled unless loss streak ≥ 3)
    cheat_checkbox = tk.Checkbutton(win, text="Cheat (Win this roll!)", variable=cheat_var, fg="red", font=("Arial", 12))
    cheat_checkbox.pack(pady=5)
    cheat_checkbox.config(state='disabled')
